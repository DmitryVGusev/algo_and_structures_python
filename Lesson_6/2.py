...
if __name__ == '__main__':
    ...
    print(f"Размер объекта через класс со словарем: {sys.getsizeof(con)}")
    print(f"Размер объекта через класс со слотами: {sys.getsizeof(con_s)}")

    """
    Класс через __dict__: 1056
    Класс через __slots__: 888
    Размер объекта через класс со словарем: 56
    Размер объекта через класс со слотами: 72
    """
    # Создается ощущение, что для элемента класса через слоты резервируется больше памяти.
